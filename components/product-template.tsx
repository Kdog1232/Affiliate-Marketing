...
function VerdictMini({ product }: { product: Product }) { return <Card><div className="flex text-blue-300">{Array.from({ length: 5 }).map((_, i) => <Star key={i} className="h-5 w-5 fill-current" />)}</div><p className="mt-4 text-3xl font-bold text-white">{product.rating}/5</p><p className="mt-3 text-slate-300">Best for {bestForText(product).toLowerCase()}.</p><CenterCta href={product.affiliateLink}>Try {product.name} →</CenterCta></Card>; }
