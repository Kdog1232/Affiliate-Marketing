...
async function generateUtilizedReview(provider: ReturnType<typeof getAiProvider>, factPack: Awaited<ReturnType<typeof buildProductFactPack>>, model: string) {
  ...
  const retryPrompt = {
    ...fullReviewPrompt(factPack),
    priorUtilizationReport: firstResult.utilizationReport,
    instructions: `${fullReviewPrompt(factPack).instructions} The prior attempt reported Knowledge Graph utilization below 90%. Regenerate from scratch and explicitly cover the omittedItems while keeping prose natural and fact-bound.`,
  };
  return provider.generateJson<EngineResponse>({ system: systemPrompt(), prompt: retryPrompt, model });
}

function needsRegeneration(report?: KnowledgeGraphUtilizationReport) {
